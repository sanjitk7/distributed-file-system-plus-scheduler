        ...
        try {
            ...
            while (true) {
                ...
                if (message.startsWith("JOIN")) {
                    membershipList.add(new Member(getMessageBody(message), senderIpAddress, senderHostName, 0, false));
                    System.out.println("Added " + senderHostName + " to membership list" );
                    logger.info("Added " + senderHostName + " to membership list" );
                    ...
                    for (int i = 0; i < membershipList.size(); i++) {
                        ...
                        if (membershipList.get(i).getId().equals(removedMembers.get(0).getId())) {
                            System.out.println("Removed " + membershipList.get(i).getHostName() + " from list" );
                            logger.info("Removed " + membershipList.get(i).getHostName() + " from list" );
                    ...
                    for (int i = 0; i < membershipList.size(); i++) {
                        if (membershipList.get(i).getId().equals(message.split(" ")[1])) {
                            System.out.println("Cleaned up " + membershipList.get(i).getHostName() + " from list" );
                            logger.info("Cleaned up " + membershipList.get(i).getHostName() + " from list" );
                    ...
                    String data = message.split("ELECTED ")[1].trim();
                    for (int i = 0; i < membershipList.size(); i++) {
                        if (membershipList.get(i).getId().equals(data)) {
                            System.out.println("Elected new leader " + membershipList.get(i).getHostName());
                            logger.info("Elected new leader " + membershipList.get(i).getHostName());
                            membershipList.get(i).setIsLeader(true);
                            break;
                        }
                    }
                }
